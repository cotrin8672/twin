        ...
        let container_id = runtime.block_on(async {
            ...
            let init_commands = vec![
                vec![
                    "sh",
                    "-c",
                    "echo '# Test Repo' > /workspace/test-repo/README.md",
                ],
            ...
            TestEnvironment::Linux => {
                ...
                runtime.block_on(async {
                    ...
                    docker
                        ...
                        .await
                        .expect("Failed to upload twin binary");
                });
            }
        }
    }
...
impl Drop for TestRepo {
    fn drop(&mut self) {
        if let TestEnvironment::Linux = &self.env {
...
/// Linuxコンテナでテストを実行するマクロ
#[macro_export]
macro_rules! test_in_container {

...
fn test_add_command_basic() {
    ...
    assert!(output.status.success(), "twin add should succeed. stderr: {}", stderr);
    ...
    assert!(list_output.contains("test-add"), "Worktree should be created");
    assert!(list_output.contains("test-branch"), "Branch should be created");
}
...
fn test_add_without_branch_option() {
    ...
    assert!(
        ...
        "Expected git error message, got: {}", stderr
    );
}
...
fn test_force_branch_option() {
    ...
    assert!(output.status.success(), "Force branch should succeed: {}", stderr);
}
...
fn test_no_checkout_option() {
    ...
    let output = Command::new(&twin)
        .args(["add", "../no-checkout", "-b", "empty-branch", "--no-checkout"])
        .current_dir(repo.path())
        .output()
        .expect("Failed to run twin add --no-checkout");
    
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(output.status.success(), "No-checkout should succeed: {}", stderr);
    ...
    assert!(!worktree_path.join("README.md").exists(), "README.md should not exist");
}
...
"#;
    ...
    // --git-onlyモードで実行
    let output = Command::new(&twin)
        .args(["add", "../git-only-test", "-b", "test-branch", "--config", ".twin.toml", "--git-only"])
        .current_dir(repo.path())
        .output()
...
fn test_list_includes_manual_worktree() {
    ...
    // 手動でgit worktreeを作成
    Command::new("git")
        .args(["worktree", "add", "../manual-worktree", "-b", "manual-branch"])
        .current_dir(repo.path())
        .output()
...
fn test_remove_manual_worktree() {
    ...
    // 手動でgit worktreeを作成
    Command::new("git")
        .args(["worktree", "add", "../manual-remove", "-b", "manual-remove-branch"])
        .current_dir(repo.path())
        .output()
        .expect("Failed to create manual worktree");
...
fn test_output_matches_git_worktree() {
    ...
    assert!(
        (git_stdout.contains("Preparing") || git_stdout.contains("HEAD") || git_stdout.contains("branch")) &&
        (twin_stdout.contains("Preparing") || twin_stdout.contains("HEAD") || twin_stdout.contains("branch")),

...
"#;
    ...
    let output = repo.run_twin(&[
        "add", "../with-symlinks", "-b", "symlink-test",
        "--config", ".twin.toml"
    ]);
    assert!(output.status.success());
    ...
    let worktree_path = match &repo.env {
        TestEnvironment::Local => {
            repo.path().parent().unwrap().join("with-symlinks")
        }
        TestEnvironment::Container => {
            std::path::PathBuf::from("/workspace/with-symlinks")
        }
    };
    ...
    let check_cmd = match &repo.env {
        TestEnvironment::Local => {
            vec!["test", "-L", worktree_path.join("config.json").to_str().unwrap()]
        }
        TestEnvironment::Container => {
    ...
    let output = repo.run_twin(&[
        "add", "../no-symlinks", "-b", "no-symlink-test",
    ...
    let check_cmd = match &repo.env {
        TestEnvironment::Local => {
            vec!["test", "-e", repo.path().parent().unwrap()
                .join("no-symlinks/config.json").to_str().unwrap()]
        }
        TestEnvironment::Container => {
    ...
    let output = repo.exec(&check_cmd);
    assert!(!output.status.success(), "Symlink should not be created with --git-only");
}
...
"#;
    ...
    let marker_check = match &repo.env {
        TestEnvironment::Local => {
            vec!["test", "-f", repo.path().parent().unwrap()
                .join("hook-marker.txt").to_str().unwrap()]
        }
        TestEnvironment::Container => {
...
fn test_complex_workflow(repo: TestRepo) {
    ...
    match &repo.env {
        ...
        TestEnvironment::Container => {
            repo.exec(&["sh", "-c", "echo 'content' > /workspace/work-1/new-file.txt"]);
            repo.exec(&["git", "-C", "/workspace/work-1", "add", "."]);
            repo.exec(&["git", "-C", "/workspace/work-1", "commit", "-m", "Work in progress"]);
        }
    }
